...
def AddStochRSI(df, indicator_list, askbidmid, timeperiod=14):
    df['rsi_fastk'], df['rsi_fastd'] = talib.STOCHRSI(df['{}_c'.format(askbidmid)].values, timeperiod=14, fastk_period=5, fastd_period=3, fastd_matype=0)
...
def VolumeWeightedPrice(df, indicator_list, askbidmid, lookback):
    df['vwap_high'] = 0
    df['vwap_low'] = 0
    df['vwap_volume'] = 0
    ...
    for index, row in df.iterrows():
        ...
        if df.index.get_loc(index) >= lookback:
            ...
            temp = df[loc-lookback+1:loc+1]
            for index2, row2 in temp[::-1].iterrows():

                sum_volume = sum_volume + row2['volume']
                sum_high = sum_high + row2['volume'] * row2['{}_h'.format(askbidmid)]
                sum_low = sum_low + row2['volume'] * row2['{}_l'.format(askbidmid)]

            df['vwap_high'].ix[index] = sum_high/sum_volume
            df['vwap_low'].ix[index] = sum_low/sum_volume
            df['vwap_volume'].ix[index] = sum_volume
