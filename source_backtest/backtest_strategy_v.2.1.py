            ...
            if date >=self.date_to_start_trading:
                ...
                if self.units_net != 0:
                    ...
                    for etrade in self.listofOpenTrades:
                        ...
                        if(etrade.unrealizedprofitloss) > 2:
                    ...
                    if ( self.data[self.decision_frequency].loc[date, 'net-move'] < 0.005 ) and ( self.data[self.decision_frequency].loc[date, 'cum-move'] / self.data[self.decision_frequency].loc[date, 'net-move'] < 5 ):
                        
                        price_ask_c, price_bid_c, price_ask_h, price_bid_h, price_ask_l, price_bid_l = self.get_price(date)
                        
                        if self.data[e_granularity].loc[date, 'mean_bid_c'] < price_bid_c:
